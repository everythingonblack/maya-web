...
import styles from './CheckmarkAnimation.module.css'; // animasi centang ✅

// Ganti dengan fungsi API milikmu

export const connectWA = async () => {
  // QR statis base64 (contoh dummy)
...
export default function WhatsappConnectModal() {
  ...
  // 🔹 Ambil QR Code saat modal dibuka
  useEffect(() => {
  ...
  // 🔹 Polling status koneksi tiap 10 detik
  useEffect(() => {
    intervalRef.current = setInterval(async () => {
      try {
        const waData = await getWAStatus();
